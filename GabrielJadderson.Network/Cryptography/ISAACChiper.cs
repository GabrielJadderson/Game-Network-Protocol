...
namespace GabrielJadderson.Network.Cryptography
{
    public class ISAACChiper
    {
        ...
        private void Seed(string seed, bool flag)
        {
        ...
        private byte[] Vernam(string msg)
        {
            int n, l;
        ...
        public string process(string key, string msg)
        {
            byte[] xctx = new byte[msg.Length];
            Seed(key, true);
            xctx = Vernam(msg);
            return System.Text.Encoding.ASCII.GetString(xctx);
        }

        public byte[] process(byte[] key, byte[] msg)
        {
            byte[] xctx = new byte[msg.Length];
            Seed(Encoding.ASCII.GetString(key), true);
            xctx = Vernam(Encoding.ASCII.GetString(msg));
            return xctx;
        }

        public uint getRandomSeeded(byte[] seed)
        {
            Seed(Encoding.ASCII.GetString(seed), true);
            return Random();
        }

        public uint getRandomSeeded(string seed)
        {
            Seed(seed, true);
            return Random();
        }
        ...
        {
            ISAACChiper isaacChiper = new ISAACChiper();

            string key = "secretKey TopSecret 42";
            string msgToSend = "Hello i am a nigger";

            byte[] enc = isaacChiper.process(Encoding.ASCII.GetBytes(key), Encoding.ASCII.GetBytes(msgToSend));

            Console.WriteLine(Encoding.ASCII.GetString(enc));

            Console.WriteLine(isaacChiper.Random());


            byte[] dec = isaacChiper.process(Encoding.ASCII.GetBytes(key), enc);
